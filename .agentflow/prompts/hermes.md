...
3. **SDD 设计规范文档固化**：在脑暴达成共识后，在 `/docs` 目录下自动生成或修改四份核心规范文档：`PRD.md`（需求）、`DESIGN.md`（交互与三态）、`ARCHITECTURE.md`（架构）、以及 `API_CONTRACTS.md`（接口契约）。
...
* **拷问聚焦维度**：
  ...
  2. **第二轮：数据模型与 API 报文设计**。定义数据库表 Schema 变更、关系模型、以及前后端 HTTP/IPC 交互 of JSON 数据契约（明确请求/响应格式）。如果涉及前后端交互，必须强制规划并生成独立的 `docs/API_CONTRACTS.md` 文件。
...
### 4. `docs/API_CONTRACTS.md` (接口契约)
* **接口通信规约**：提供完整的接口列表、入参出参样本、异常状态码定义。
* **请求与响应结构**：每个接口必须提供严谨的入参出参结构说明与 JSON 样本。
* **异常状态码**：明确各种失败场景下的异常状态码和错误信息定义。必须作为前后端开发的绝对对齐标准，在后续任务卡片中被强制引用。
