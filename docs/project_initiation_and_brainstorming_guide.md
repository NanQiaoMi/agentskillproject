...
- **Superpowers 核心赋能**：在脑暴访谈 and 设计中，你必须应用你的 **Superpowers** 核心赋能技能，通过 20 多个可组合的 Skill 覆盖开发全流程来进行深度的 Brainstorming 与系统规划（在计划阶段特别优先运用 `brainstorming` 和 `writing-plans` 等 Skill 来规范系统设计和落地步骤）。
...
- **深度聚焦范围与设计规划**：访谈应当主动深挖那些用户可能会忽略的隐藏难点和盲区，避免询问浅显表面的问题。必须将 **frontend-design**（高品质、高保真的前端视觉设计，规避平淡简陋的 AI 样式）与 **ui-ux-pro-max**（全面的 UI/UX 规范，如设计风格、配色系统、字体排版、微交互等）的理念融入规划。必须聚焦于：
  - **技术栈选型与可行性分析**（如：服务器成本评估、三方 API 依赖、安全性隔离等）。
  - **交互逻辑与设计表现细节**（如：前端组件复用规范、三态视觉表现（加载中 Loading、数据为空 Empty、接口/网络报错 Error）、关键流程的分叉和异常中断路径）。
...
- **文档多轮迭代与确认**：持续迭代提问，直至所有关键技术细节确认完毕。AI 自动整理出第一版详细、可落地的开发规范文档（PRD / DESIGN / ARCHITECTURE 草稿），在此设计文档中必须明确规划符合 **frontend-design** 和 **ui-ux-pro-max** 的设计令牌（Design Tokens，包括颜色、间距、字号等 CSS 变量）、前端版式布局、交互动效等，并根据用户的反馈反复进行多轮修改优化，直至用户完全满意。
