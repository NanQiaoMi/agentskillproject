...
def cmd_brainstorm(args):
    ...
    print("在这个过程中，你必须应用你的 **Superpowers** 核心赋能技能，通过 20 多个可组合的 Skill 覆盖开发全流程来进行深度的 Brainstorming 与系统规划（特别优先运用 `brainstorming` 和 `writing-plans` 等 Skill 来规范系统设计和落地步骤）。同时，你必须将 **frontend-design**（高品质、高保真的前端视觉设计，规避平淡简陋的 AI 样式）与 **ui-ux-pro-max**（包括设计令牌 Design Tokens 如 CSS 变量、布局排版、微交互风格、配色系统等）的理念融入整个规划中。\n")
    print("我的初始创意为：[在此处填写您的创意，例如：实现‘找回/重置密码’功能]\n")
    print("请深挖我可能忽略的交互和技术死角，分轮次提问，每轮只提出 1-2 个最关键的问题。请聚焦技术栈选型、交互逻辑与设计表现细节（包括前端组件复用、三态表现（Loading/Empty/Error）、视觉风格和配色方案等）、边界异常场景以及潜在的技术难点与盲区，避免询问浅显表面问题。\n")
    print("请持续迭代提问，直至所有关键细节确认完毕，最终输出一份符合 frontend-design 和 ui-ux-pro-max 规范的完整、可落地的详细项目开发文档（包括 docs/PRD.md, docs/DESIGN.md, docs/ARCHITECTURE.md 草案），并根据我的反馈反复优化，直到我完全满意。内容需要经过多次迭代。\n")
    print("现在，请向我提第一轮问题。")
    print("---------------------------------- 复制上方提示词 ----------------------------------\n")
