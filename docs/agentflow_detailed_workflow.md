...
graph LR
    ...
    subgraph 智能体角色域
        ...
        codex[codex 后端]
        claudecode[claudecode 审查与修复]
    end
    antigravity -->|只写范围| FE[src/frontend/*]
    codex -->|只写范围| BE[src/backend/*]
    claudecode -->|全局读写| Global[全局代码/测试/修复]
```
...
| **claudecode** | 代码审查与修复智能体 | 全局（仅限微调/测试） | 全局项目范围 | `review --approve`, `review --reject`, `review --env-fail` |
...
- 自动执行物理文件暂存与本地 Commit，以保证代码不遗失在工作区中：
  ...
  ```
- 自动更新任务状态为 `review` 并将指派人移交为 `claudecode`。
...
审查智能体 `claudecode` 通过配置文件 `.agentflow/config.json` 获取针对对应开发角色的检验命令：
...
- **若测试全部通过 (Exit Code 0)**：`claudecode` 可顺利审批通过。
- **若因代码缺陷失败 (Exit Code != 0)**：`claudecode` 自动触发 `review --reject` 并提供详细调用栈。
- **若因本地环境故障失败（如缺少包、接口冲突）**：`claudecode` 执行 `review --env-fail` 挂起任务，防止无效的代码修复循环。
