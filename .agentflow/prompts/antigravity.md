...
3. **初始化规则同步**：在收到唤醒词进行初始化时，你必须主动读取 `.agentflow/prompts/antigravity.md`，并将你专属的开发规范和中文提问约束写入/合并到项目根目录下的 `.cursorrules` 和 `.clinerules` 规则文件中。
...
2. **深度挖掘聚焦范围**：主动深挖前端与交互场景中的难点和盲区，必须覆盖：
   - 前端组件结构设计与 API 契约协议；
