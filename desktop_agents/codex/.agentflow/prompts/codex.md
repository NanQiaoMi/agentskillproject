...
2. **深度挖掘聚焦范围与设计规划**：主动深挖后端与前端设计交互的难点和盲区，必须将 **frontend-design**（高品质、高保真的前端视觉设计，规避平淡简陋的 AI 样式）与 **ui-ux-pro-max**（全面的 UI/UX 规范，如设计风格、配色系统、字体排版、微交互等）的理念融入规划。访谈和需求挖掘必须覆盖：
   - 数据模型与表 Schema 设计、服务层接口契约定义；
   - 交互三态对应的后端响应规范（如：错误状态码、Loading 延迟与空数据体设计，配合前端组件复用规范）；
   - 边界异常场景（弱网重试、并发竞争锁、防重复提交校验、接口限频、空值溢出处理）；
   - 前后端拟配合采用的 `ui-ux-pro-max` 视觉风格（如 Bento Grid、Neumorphism、Glassmorphism 等）、HSL/RGB 配色方案及高级字体层级；
   - 潜在的技术难点、系统安全授权与测试真值（Ground Truth）定义。
3. **SDD 文档编写与迭代**：基于访谈共识，在项目的 `docs/` 目录下编写或更新详细的开发规范文档（`docs/PRD.md`、`docs/DESIGN.md`、`docs/ARCHITECTURE.md`）。在设计文档中，必须明确规划符合 **frontend-design** 和 **ui-ux-pro-max** 的设计令牌（Design Tokens，包括颜色、间距、字号等 CSS 变量）、前端版式布局、交互动效以及后端接口契约。根据用户反馈进行多轮修改迭代，直至用户完全满意。
